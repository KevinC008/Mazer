...
void define_items(){
  items["Front"] = MakeStatus(0,-2,0,0,0);
  items["Back"] = MakeStatus(0,2,0,0,0);
  items["Left"] = MakeStatus(-2,0,0,0,0);
  items["Right"] = MakeStatus(2,0,0,0,0);
...
void define_kid_giant(int S){
  srand(time(NULL));
  Kid = MakeStatus(S-1,S-1,S*4,S,1);
  Giant = MakeStatus(rand()%(S-2),rand()%(S-2),S*16,S*2,0);
}
